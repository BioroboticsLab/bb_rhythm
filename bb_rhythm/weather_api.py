...
def get_air_temperature_frame(dt_from, dt_to, station_name="Berlin-Tempelhof"):
    ...
    stations = DwdObservationRequest(parameter=[("temperature_air_mean_200")],
                                     resolution=DwdObservationResolution.MINUTE_10,
                                     start_date=dt_from,
                                     end_date=dt_to,
                                     settings=settings).filter_by_name(name=station_name)
    temp_lst = []
...
def get_wind_speed_frame(dt_from, dt_to, station_name="Berlin-Tempelhof"):
    settings = Settings(tidy=True, si_units=False, humanize=True)
    stations = DwdObservationRequest(parameter=[("wind_speed")],
                                     resolution=DwdObservationResolution.MINUTE_10,
                                     start_date=dt_from,
                                     end_date=dt_to,
                                     settings=settings).filter_by_name(name=station_name)
...
def combine_temp_wind_frames(temp_frame, wind_frame):
    frame = temp_frame.merge(wind_frame, left_on=['date', 'station_id'], right_on=['date', 'station_id'],
                             suffixes=('_temp', '_wind'))
    frame.drop(
        columns=['dataset_temp', 'parameter_temp', 'quality_temp', 'dataset_wind', 'parameter_wind', 'quality_wind'],
        inplace=True)
    frame.rename(columns={'value_temp': "temperature_air_mean_200", 'value_wind': 'wind_speed_mean'}, inplace=True)
    return frame
